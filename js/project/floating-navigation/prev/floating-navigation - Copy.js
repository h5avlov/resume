...
function floatingNavigationInit( linksContainerCopy, targetContainer, classNames ) {
	...
	const floatingNavComponents = floatingNavigationBuilding( linksContainerCopy, targetContainer, classNames ), 
		{ floatingNav, buttonFloating, buttonClose } = floatingNavComponents;
	
	const labelInteraction = labelInteractionInit( labelVisibleClass );
	
	const floatingNavHandlers = floatingNavHandlers( floatingNav, classNames ); 
	const navLinksHandlers = navLinksHandlers( labelInteraction );
	const linksHandlers = linksHandlers( labelInteraction, utilities.nav.navigationClickHandler, utilities.nav.getTargetSectionPos ); 
	const buttonOpenHandlers = buttonOpenHandlers( buttonFloating, {
		minScrollDown, 
		buttonTimeOn, 
		buttonVisibleClass
	} );
	const buttonCloseHandlers = buttonCloseHandlers( labelInteraction, floatingNav.close );
	
	
	
	$( document ).on( "scroll", function() {
		buttonOpenHandlers.scroll( $( document ).scrollTop() );
		floatingNavHandlers.eventHandlers.scroll();
	} );
